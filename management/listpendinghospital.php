      ...
      //Delete the selected record 
        function deleteUser(Id, UserName)
        {
          ...
          if (r == true)
          {
            ...
            ajaxRequest.open("GET", "listpendinghospital.php" + queryString, true);
...
function displayContent($UserType) {
  $strContent = '<h3>Pending Hospitals</h3>';
  if ($UserType == "GMO") {
    ...
    $result = mysql_query("SELECT user.username, usertype, user.status 
			FROM user
			LEFT JOIN hospital on user.username = hospital.username  
			where user.status='Pending' and hospital.districtid='" . $districtid . "' ")
        or die(mysql_error());
    $paginationQuery = "SELECT user.username, usertype, user.status 
											FROM user
												LEFT JOIN hospital on user.username = hospital.username  
												where user.status='Pending' and hospital.districtid='" . $districtid . "'";
  }
    ...
    $userName = $_SESSION['userName'];
    $resultGmo = mysql_query("SELECT districtid FROM dao where username='" . $userName . "' ")
        or die(mysql_error());
    $rowDao = mysql_fetch_array($resultGmo);
  ...
  if ($intResultNum > 0) {
    ...
    while ($row = mysql_fetch_array($result)) {
      ...
      $strContent.="<td class=\"tdContentImg\">
				<a href=\"viewuser.php?hosApp=" . $row['username'] . "\">
