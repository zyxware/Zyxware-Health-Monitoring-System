      ...
      //Delete the selected record
        ...
        function deleteUser(Id, name)
        {
          ...
          if (r == true)
          {
            ...
            ajaxRequest.open("GET", "listdao.php" + queryString, true);
...
function displayContent($authorise) {
  ...
  if ($authorise == "GMO") {
    ...
    $result = mysql_query("SELECT user.status, dao.username, daoid, name, designation,
													emailid,phonenumber, mobilenumber,districtid
													FROM
													dao
														LEFT JOIN user on dao.username= user.username
														where user.status='Approved' AND dao.districtid='" . $districtid . "' ")
        or die(mysql_error());
    $paginationQuery = "SELECT user.status, dao.username, daoid, name, designation,
													emailid,phonenumber, mobilenumber,districtid
													FROM
													dao
														LEFT JOIN user on dao.username= user.username
  ...
  if ($intResultNum > 0) {
    ...
    $strContent.='<table class="listContentTab" id="tblList">
			<tr>
				...
				<th class="tdBorder">Phonenumber</th>
